...
function GameGrid() {
    ...
    const [searchResults, ] = useState<Game[] | null>(null);

    const userId = auth.currentUser?.uid;

    // ✅ Fixed: Memoized fetchData with proper type handling
    const fetchData = useCallback(async (pageNumber: number) => {
        try {
            ...
            const formattedGames: Game[] = data.results.map((game: RawgGameResponse) => ({
                id: game.id,
                title: game.name,
            ...
            <main className="overflow-x-hidden relative z-20">
                ...
                <section className="space-y-4 pt-4">
                    <h2 className="text-xl font-semibold">
                        {searchResults ? "Search Results" : selectedCategoryName || "All Games"}
                    </h2>
